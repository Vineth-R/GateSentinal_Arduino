...
void setup() {
  ...
  if (!finger.verifyPassword()) {
    textDisplay("Fingerprint sensor error!");
...
void enrollFingerprint(int id, String username) {
  ...
  while (retryCount < maxRetries) {
    ...
    finger.LEDcontrol(FINGERPRINT_LED_ON, 0, FINGERPRINT_LED_PURPLE, 0);
    textDisplay("Remove finger");
    Serial.println("Please remove your finger.");
    delay(1500);

    for (int i = 0; i < 8; ++i) {
      finger.LEDcontrol(FINGERPRINT_LED_ON, 0, FINGERPRINT_LED_BLUE, 0);
      textDisplay("Place same finger (2/2)");
      if (i == 0)
    ...
    if (result != FINGERPRINT_OK) {
      finger.LEDcontrol(FINGERPRINT_LED_ON, 0, FINGERPRINT_LED_RED, 0);
      textDisplay("Model create error");
    ...
    if (result == FINGERPRINT_OK) {
      ...
      finger.LEDcontrol(FINGERPRINT_LED_ON, 0, FINGERPRINT_LED_RED, 0);
      textDisplay("Enroll/store error");
  ...
  textDisplay("Enroll failed. Try again.");
  delay(2000);
  finger.LEDcontrol(FINGERPRINT_LED_ON, 0, FINGERPRINT_LED_PURPLE, 0);
}
...
void recognizeFingerprint() {
  ...
  for (int i = 0; i < 8; ++i) {
    finger.LEDcontrol(FINGERPRINT_LED_ON, 0, FINGERPRINT_LED_BLUE, 0);
    textDisplay("Place finger to check");
    delay(250);
    finger.LEDcontrol(FINGERPRINT_LED_ON, 0, FINGERPRINT_LED_PURPLE, 0);
    delay(250);
  }
...
//Get fingerprint image
  ...
  if (result == FINGERPRINT_OK) {
    ...
    finger.LEDcontrol(FINGERPRINT_LED_ON, 0, FINGERPRINT_LED_BLUE, 0);
    textDisplay("ID: " + String(matchedID) + "\nName: " + name);
